...
```bash
# 1) 解包
tar xzf fulla-sdk-1.1.1-linux-x86_64.tar.gz   # -> fulla-sdk-1.1.1-linux-x86_64/
...
cmake -S . -B build \
  ...
  -DCMAKE_PREFIX_PATH=$PWD/fulla-sdk-1.1.1-linux-x86_64
cmake --build build -j
```
...
```bash
docker pull ghcr.io/voidvec/fulla-backend:1.1.1
```
...
3. Create a strict SemVer tag: `git tag v1.1.1 && git push origin v1.1.0`. Tags with a suffix (e.g. `v1.0.0-rc1`) do **not** trigger a release.
